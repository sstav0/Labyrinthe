...
@timeit
def negamaxPruning(board: list, tile: dict, playersPos: list[int], playerIndex: int, targetId: int, depth: int = 3, alpha=float("-inf"), beta=float("inf"), timeLimit: float = 3.0) -> tuple:
    """
    ...
    for move in legalMoves(current_board.getBoard(), current_board.getPos(), current_board.getFreeTile()):
        ...
        value, _ = negamaxPruning(child_board, freeTile, playersPos,
                                  playerIndex % -2 + 1, targetId, depth-1, -beta, -alpha, timeLimit)
